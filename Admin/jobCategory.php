      ...
      <p id="deleteAlertText"></p>
        ...
        <div class="table-responsive" >
            <table class="table table-striped table-sm " data-show-columns="true" data-height="350" id="jCat" data-unique-id="id">
              ...
              <tbody>
                <?php 
                $sql= "select j.*,s.name as managerName from job_category as j left join staff as s on j.manager_id =s.nic order by j.isactive desc ";
                ...
                <tr>
                  ...
                  <td <?php echo $clsTextMuted; ?> ><?php echo $row['description']; ?></td>
                  <td><?php 
                  if($row['managerName']!="")
...
<div class="modal fade" id="deleteDataModal" role="dialog">
    <div class="modal-dialog modal-md">
      <div class="modal-content">
        <div class="modal-header">          
          <h5 class="modal-title">Job Category Activation</h5>
        </div>
        <form method="post" action="" class="needs-validation" novalidate>
        <div class="modal-body">
          <div class="col-12 mb-3">
...
if(isset($_POST['deletem'])){
  try{
    ...
    if($activeStatus==0){
      $sql="update job_category set isactive=1, manager_id=NULL where id=$id ";
    }else{
    ...
    if(mysqli_query($con,$sql)){
      ...
      document.getElementById('deleteAlertText').innerHTML='User activated';
      </script>";
      echo "<script>window.location.href='jobCategory.php';</script>";
      exit;
    }else{
